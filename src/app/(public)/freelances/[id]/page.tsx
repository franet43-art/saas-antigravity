...
import { ChevronLeft, MessageCircle, Clock, User, ShieldCheck } from 'lucide-react';
...
export default async function FreelanceDetailPage({ params }: PageProps) {
  ...
  const { data: profile } = await supabase
    .from('profiles')
    .select('id, full_name, bio, category, custom_category, hourly_rate')
    .eq('id', id)
    .eq('role', 'freelance')
    .eq('status', 'active')
    .single();
  ...
  return (
    <div className="container mx-auto py-12 px-4 max-w-4xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
